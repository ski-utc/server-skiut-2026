...
class AdminController extends Controller
{
    ...
    */
    ...
    /**
     * Récupère tous les utilisateurs pour la gestion des membres
     */
    public function getUsers()
    {
        try {
            $users = User::select('id', 'firstName', 'lastName', 'email', 'member', 'roomID', 'admin')
                ->orderBy('lastName')
                ->orderBy('firstName')
                ->get();
    ...
    /**
     * Met à jour le statut de membre d'un utilisateur
     */
    public function updateUserMemberStatus(Request $request)
    {
        try {
            $request->validate([
            ...
            $memberStatus = $request->input('member');

            $user = User::findOrFail($userId);
            $user->member = $memberStatus;
            $user->save();

            return response()->json([
                'success' => true,
                'message' => $memberStatus ?
                    'Statut de membre accordé avec succès' :
                    'Statut de membre retiré avec succès'
            ]);
